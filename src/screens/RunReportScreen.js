...
export default function RunReportScreen() {
  ...
  const handleViewReport = async () => {
    ...
    try {
      ...
      setData([]);
      Alert.alert("Error", error?.response?.data?.message || "Failed to load run report");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async (format) => {
    ...
    try {
      setExporting(true);
      if (format === "csv") {
        await downloadReportAsCsv(rows, `run-report-${stamp}.csv`);
      } else {
        await downloadReportAsXlsx(rows, `run-report-${stamp}.xlsx`);
      }
    } catch (error) {
                ...
                <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
                  <View>
                    ...
                    <Text style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase" }}>
                      {normalizeDisplayValue(row.orgName) ||
                        (row.orgId ? accountNameById.get(Number(row.orgId)) || `Org ${row.orgId}` : "Run Report")}
                    </Text>
                  </View>
